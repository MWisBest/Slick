...
public class MipmapTest extends BasicGame
{
	...
	private Image createMipmapImage( String ref ) throws SlickException
	{
		...
		try
		{
			...
			Texture tex = InternalTextureLoader.get().createTexture( imageData, // the image data holding width/height/format
	...
	{
		Input in = c.getInput();
		if( in.isKeyDown( Input.KEY_UP ) ) scale = Math.min( 2f, scale + 0.001f );
