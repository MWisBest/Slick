...
public class ShaderTestAdvanced extends BasicGame
{
	...
	public void render( GameContainer container, Graphics screenGraphics ) throws SlickException
	{
		...
		if( shaderWorks ) screenGraphics.drawString( "H to toggle horiz" + ( useHoriz ? " (enabled)" : "" ) + "\nV to toggle vert" + ( useVert ? " (enabled)" : "" ) + "\nUP/DOWN to change radius: " + radius + "\nQ/A to change alpha blending: " + alpha, 10, 25 );
		else if( !supported ) screenGraphics.drawString( "Your drivers do not support OpenGL Shaders, sorry!", 10, 25 );
		else screenGraphics.drawString( "Oops, shader didn't load!", 10, 25 );
		if( log != null && log.trim().length() != 0 ) screenGraphics.drawString( "Shader Log:\n" + log, 10, 75 );
	}
