...
public class ShaderTest extends BasicGame
{
	...
	private void reload()
	{
		...
		try
		{
			...
			log = program.getLog();
			if( log != null && log.length() != 0 ) Log.warn( log );
	...
	public void render( GameContainer container, Graphics g ) throws SlickException
	{
		...
		if( shaderWorks && useShader ) program.bind();
		
		g.drawImage( logo, 100, 300 );
		
		// unbind the shader so that usual stuff renders OK
		if( shaderWorks && useShader ) program.unbind();
		
		if( shaderWorks ) g.drawString( "Space to toggle shader\nPress R to reload shaders", 10, 25 );
		else if( !supported ) g.drawString( "Your drivers do not support OpenGL Shaders, sorry!", 10, 25 );
		else g.drawString( "Oops, shader didn't load!", 10, 25 );
		if( log != null && log.length() != 0 ) g.drawString( log, 10, 75 );
	}
	
	@Override
	public void update( GameContainer container, int delta ) throws SlickException
	{
		if( container.getInput().isKeyPressed( Input.KEY_SPACE ) ) useShader = !useShader;
		if( container.getInput().isKeyPressed( Input.KEY_R ) ) reload();
	}
}