...
public class FontTest extends BasicGame
{
	...
	public void render( GameContainer container, Graphics g )
	{
		...
		font.drawString( 100, 36 + font.getHeight( "We Here is a more complete line that hopefully" ), "will show some kerning." );
		...
		font2.drawString( 100, 136 + font2.getHeight( "We - Here is a more complete line that hopefully" ), "will show some kerning." );
		image.draw( 100, 400 );
	...
	private void drawTextBox( AngelCodeFont font, CharSequence text, float x, float y, float maxWidth )
	{
		...
		for( int i = 0; i < text.length(); i++ )
		{
			...
			if( def.xoffset + def.width + x > maxWidth ) break;
	...
	public void update( GameContainer container, int delta ) throws SlickException
	{
		...
		if( width > 270 || width < 0 ) widthMult = widthMult == 1 ? -1 : 1;
	}
