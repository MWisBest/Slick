...
public class ShaderProgram
{
	...
	public ShaderProgram( String vertexShaderSource, String fragShaderSource ) throws SlickException
	{
		...
		if( !isSupported() ) throw new SlickException( "no shader support found; driver does not support extension GL_ARB_shader_objects" );
	...
	protected int createProgram() throws SlickException
	{
		...
		if( program == 0 ) throw new SlickException( "could not create program; check ShaderProgram.isSupported()" );
		return program;
	}
	...
	protected int compileShader( int type, String source ) throws SlickException
	{
		...
		if( shader == 0 ) throw new SlickException( "could not create shader object; check ShaderProgram.isSupported()" );
		...
		if( err != null && err.length() != 0 ) log += t + " compile log:\n" + err + "\n";
		if( comp == GL11.GL_FALSE ) throw new SlickException( log );
		return shader;
	}
	...
	{
		if( !valid() ) throw new SlickException( "trying to link an invalid (i.e. released) program" );
	...
	public int getUniformID( String name )
	{
		...
		if( location == -1 && strict ) throw new IllegalArgumentException( "no active uniform by name '" + name + "' (disable strict compiling to suppress warnings)" );
	...
	public int getAttributeID( String name )
	{
		...
		if( location == -1 && strict ) throw new IllegalArgumentException( "no active attribute by name '" + name + "'" );
	...
	{
		if( buf4 == null ) buf4 = BufferUtils.createFloatBuffer( 4 );
		buf4.clear();
